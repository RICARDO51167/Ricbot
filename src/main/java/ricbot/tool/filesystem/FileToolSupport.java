...
public final class FileToolSupport {
    ...
    public static void ensureAllowed(Path path, Path allowedDir, List<Path> extraAllowedDirs) {
        ...
        throw new IllegalArgumentException("Path is outside allowed directory: " + normalized);
    }
    ...
    public static String formatDirList(Path dir, List<Path> items) {
        StringBuilder sb = new StringBuilder();
        sb.append("Directory: ").append(dir.toAbsolutePath().normalize()).append("\n\n");
        ...
        for (Path item : items) {
            ...
            if (!isDir) {
                try {
                    sb.append(" (").append(Files.size(item)).append(" bytes)");

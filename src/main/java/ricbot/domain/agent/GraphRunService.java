...
/**
 * 单次 Run 的唯一顶层调度器：拥有 Graph 状态迁移，并协调 Model、Tool、Checkpoint 与 Run Journal。
 */
public class GraphRunService implements AutoCloseable {
    ...
    private static final Logger log = LoggerFactory.getLogger(GraphRunService.class);
    ...
    private final ModelNodeExecutor modelNodeExecutor;
     ...
     */
    public GraphRunService(LLMProvider provider) {
     ...
     * 可注入执行器版本，方便测试与资源治理。
     */
    public GraphRunService(LLMProvider provider, ExecutorService toolExecutor, boolean ownsToolExecutor) {
        this.modelNodeExecutor = new ModelNodeExecutor(provider);
        this.toolNodeExecutor = new ToolNodeExecutor(
                toolExecutor != null ? toolExecutor : SHARED_TOOL_EXECUTOR, ownsToolExecutor);
    ...
    private LLMResponse requestModel(
        ...
        if (!forceNonStreaming && hook != null && hook.wantsStreaming()) {
            return modelNodeExecutor.execute(spec, messages, toolDefinitions, true,
                    delta -> safeHook(
        ...
        return modelNodeExecutor.execute(spec, messages, toolDefinitions, false, null, null);
    }
    ...
    private RuntimePolicy resolveRuntimePolicy(
        ...
        exposedToolsBeforeCapability = definitions.stream()
                .map(GraphRunService::schemaName)
    ...
    private void publishCheckpoint(
        ...
        checkpointService.publish(spec, runId, journalRunId, journalSequence, iteration, phase,
                nodeState, messages, checkpointMessageOffset, assistantMessage,
                completedToolResults, pendingToolCalls);
    }
    ...
    private List<Map<String, Object>> executeToolsConcurrent(
        ...
        CompletionService<IndexedToolExecution> cs = toolNodeExecutor.completionService();
